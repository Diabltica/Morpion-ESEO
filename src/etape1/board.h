 ...
 */
typedef enum {
  NONE,  /**< used to tell there is no piece (a piece that has no kind is not a
            piece) */
 ...
 */
typedef enum {
  PIECE_IN_PLACE,     /**< no problem: the piece has been put in the specified
                         square */
  SQUARE_IS_NOT_EMPTY /**< the specified square is not empty, then the piece has
                         not been put */
} PutPieceResult;
 ...
 */
typedef void (*SquareChangeCallback)(Coordinate x, Coordinate y,
                                     PieceType newContent);
 ...
 *
 * @param [in] onSquareChange address of the function that will be called each
 * time a square changes (can be NULL if unused)
 * @param [in] onEndOfGame address of the function that will be called when the
 * end of game happens
 */
extern void Board_init(SquareChangeCallback onSquareChange,
 ...
 * @retval SQUARE_IS_NOT_EMPTY the piece cannot be put in the square as there is
 * already one
 */
